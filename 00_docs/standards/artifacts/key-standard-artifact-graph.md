...
원천 문서(2)를 capability, code, API, test artifact와 연결하기 위한 Keystone의 관계 계층이다.
...
7. `key.test.*`: capability, API, behavior를 검증하는 test artifact
...
5. 중요한 regression 또는 contract test다.
6. 여러 작업서(4)에서 반복 참조될 가능성이 높다.
...
1. `required`: 직접 typed relation, 명시된 acceptance criteria, 직접 code/test dependency로
   반드시 검토해야 한다.
