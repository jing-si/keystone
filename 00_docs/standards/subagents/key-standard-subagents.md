...
3. 사용자가 승인하지 않은 원천 문서(2), code, config, schema, API, test, generated/codegen file 변경
...
8. External coding skill: Code/config/schema/API/test 수정 workflow를 돕는 외부 보조 스킬(12)이다.
...
3. `implement`
   ...
   - 출력: 변경 파일, changed artifact IDs, reuse decision, code/config/schema/API/test anchor 변경,
     verification result, 남은 risk
   ...
   - 재사용: 새 기능이나 method를 만들기 전에 capability, `provides`, `uses`, `implements`,
     실제 code reference, 기존 test를 함께 검색하고 reuse decision을 보고한다. 기본 decision은
     `reuse_existing`, `extend_existing`, `create_new`를 사용한다. 유사 구현이나 중복 후보 때문에
     shared module 추출 또는 추가 조사가 필요하면 `extract_shared_module` 또는
     `investigate_first`를 보고할 수 있다.
   - `extract_shared_module`은 shared architecture, public interface, API/schema/test scope에 영향을
     줄 수 있으므로 worker가 직접 확정하지 않고 `NEEDS_SCOPE_CHANGE` 또는 `NEEDS_CONTEXT`로
     보고한다.
   - `investigate_first`는 evidence가 부족하다는 report이며 구현 허가가 아니다. Coordinator는
     필요하면 Linker report, Clarify modularization decision topic, 또는 investigate assignment로
     넘긴다.
