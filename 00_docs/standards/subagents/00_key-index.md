...
1. `key-standard-subagents.md`
   - Keystone helper/subagent의 lane, role, authority, role catalog, report status,
     branch/merge authority, 공통 금지와 stop condition을 정의한다.
...
<!-- key: id=key.standard.subagent.index.reading-rules refs=key.role.subagent key.doc.standard key.topic.work-execution key.topic.merge-gate -->
## 읽기 규칙

1. Subagent role, report status, merge authority를 정의하거나 변경할 때는
   `../01_key-project-standard.md`의 `STD-KEYSTONE-030`, `STD-KEYSTONE-031`,
   `STD-KEYSTONE-032`, `STD-KEYSTONE-034`, `STD-KEYSTONE-043`을 먼저 확인한다.
