...
capability(16), code artifact, API, test artifact, AI 실행 결과를 하나의 추적 가능한
작업 흐름으로 연결하는 것이다.

Keystone에서 문서는 단순 산출물이 아니라 사람, AI, 프로젝트가 공유하는 좌표계다.
기준서(3)와 작업서(4)는 의도와 규칙을 보존하고, 아티팩트 그래프(14)는 문서와 실제
code/test 사이의 연결과 영향 후보를 드러내며, execution packet은 외부 실행자가 같은 의도와
경계 안에서 작업하도록 context를 제공한다.
...
3. `keystone-author`: 수락된 결정(6)과 범위 안에서 원천 문서(2)와 Change Set(17)을 작성하거나
   수정한다.
4. `keystone-linker`: 문서, capability(16), code, API, test artifact의 연결과 impact/stale
   후보를 탐색한다.
...
- Keystone execution packet과 execution report contract
...
(19) Execution packet(execution packet): Keystone context를 외부 실행자가 수행할 수 있는
bounded 작업 입력으로 압축한 runtime output
...
- 사용 규칙: 원천 문서(2)가 아니며, Keystone 기준서와 작업서에서 복구한 context를 외부
  executor에게 전달하기 위한 파생 runtime output이다.

(20) Execution report(execution report): 외부 실행자 또는 code-worker가 수행 결과를 Keystone
workflow로 되돌리는 runtime output
...
6. 문서, capability(16), code, API, test artifact의 연결, impact candidate, stale metadata
   후보 탐색은 `keystone-linker`로 routing한다.
7. Keystone-managed 작업서(4)의 step 실행, execution packet/report, report handling, review,
   verification, acceptance flow는 `keystone-coordinator`로 routing한다.
8. Legacy uppercase `KEY-` 또는 `_KEY-` 파일은 filename migration 전까지 Keystone-managed
   문서로 읽을 수 있지만, 새 파일과 rename 대상 파일은 lower-case key prefix(11)를 사용한다.
9. key prefix(11)가 없는 일반 프로젝트 문서는 자동으로 Keystone-managed 문서로 판단하지
   않는다.
10. 외부 보조 스킬(12)은 사용자가 호출했거나 수락된 Keystone 기준서 또는 작업서가 명시적으로
   허용한 경우에만 사용한다.
...
9. 문서, capability(16), code, API, test artifact의 impact/stale 후보 해석은
   `keystone-linker` 기준서를 따른다.

Keystone 작업은 문서 탐색, impact review, context brief, execution packet, review,
...
판단할 수 있지만, 깊은 artifact impact/stale 해석은 `keystone-linker`로 넘긴다. Reader는
...
키메타/index/progress 반영 범위는 Author가 소유한다. code/test impact candidate의 깊은 탐색은
`keystone-linker` report를 입력으로 사용한다. 상세 계약은
...
6. Change Set: 큰 문서 변경, code/test 영향이 있는 문서 변경, 여러 artifact에 걸친 변경은
   추적 가능한 Change Set(17)으로 정리한다.
7. Impact candidates: 문서 변경이 code, API, capability(16), test artifact에 영향을 줄 수
...
<!-- key: id=key.standard.project.std-keystone-043 refs=key.topic.skill-contract key.output.execution-packet key.output.execution-report -->
### STD-KEYSTONE-043: Coordinator는 execution context broker다

`keystone-coordinator`는 작업서(4)의 Goal unit, Reader context brief, Linker report,
Author가 작성한 Change Set(17)을 바탕으로 execution packet(19)을 만들고, Codex,
Superpowers, bounded subagent, 사람, 기타 executor의 execution report(20)를 Keystone
workflow로 회수하는 context broker다. 코드 작업은 대표적인 실행 작업이지만, Coordinator는
코딩 방법론 자체를 독점하지 않는다. 상세 계약은 `skills/coordinator/key-standard-coordinator.md`를
따른다.

1. Assignment: Current Step Brief, Context Pack, handoff boundary, reviewer focus를 준비한다.
2. Execution packet: 외부 executor가 수행할 goal, intent, artifact context, allowed scope,
   forbidden changes, verification, return report contract를 만든다.
3. Executor routing: Codex, Superpowers, bounded subagent, 사람, 기타 executor 중 승인된
   경로로 넘긴다.
4. Execution report handling: report status를 progress status와 분리해 처리한다.
5. Review/verification: Linker report, execution report, 기준서-led verification을 기준으로
   review와 verification을 조율한다.
6. Merge coordination: merge authorization과 final acceptance를 분리한다.
...
### STD-KEYSTONE-044: Linker는 artifact relation과 impact/stale 후보를 해석한다

`keystone-linker`는 read-only로 원천 문서(2), 키메타(9), 코드 앵커(18), repository evidence를
함께 사용해 문서, capability(16), code, API, test artifact의 연결과 영향 후보를 탐색한다.
Linker는 artifact graph의 해석과 후보 보고를 소유하지만 원천 문서(2), code, config, test를
직접 수정하지 않는다.

1. Artifact discovery: 현재 요청과 관련된 capability, API, code, test artifact 후보를 찾는다.
2. Impact analysis: 문서, 결정(6), code diff, test diff가 다른 artifact에 줄 영향 후보를
   등급화한다.
3. Stale review: locator, relation, metadata gap, semantic mismatch 후보를 보고한다.
4. Execution context seed: Coordinator가 execution packet(19)을 만들 수 있게 artifact context와
   reuse/test/stale 후보를 정리한다.
5. Handoff: 문서 metadata 수정은 Author에게, code/test metadata 수정이나 implementation은
   Coordinator workflow로 넘긴다.
...
2. Document reading
   - Reader는 관련 기준서와 작업서만 읽고, 관련 없는 branch를 불러오지 않으며, 필요한 context
     brief를 만든다.
...
5. Artifact linking
   - Linker는 문서, capability(16), code, API, test artifact의 영향 후보와 stale 후보를
     read-only로 보고한다.
6. Work execution
   - Coordinator는 작업서 step과 Linker report를 execution packet(19)으로 만들고 execution
