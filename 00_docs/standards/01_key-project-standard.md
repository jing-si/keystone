...
목표는 사람이 읽을 수 있는 원천 문서(2)를 중심 인터페이스로 삼아 사람의 의도, 문서의 규범,
코드의 실제 구현, 테스트의 검증 증거를 아티팩트 그래프(14)로 연결하는 것이다.
...
- 키메타(9)와 코드 앵커(18)로 연결되는 code, API, capability, test artifact
...
- 의미: 문서에 담을 후보 내용이나 판단 재료를 만들 수 있지만 Keystone 문서 형식, 기록 위치,
  상태 의미, metadata 규칙을 대신 정하지 않는다.
- 사용 규칙: 사용자가 명시적으로 호출했거나 수락된 Keystone 기준서(3) 또는 작업서(4)가
  허용한 경우에만 보조 input으로 사용한다.
...
(14) 아티팩트 그래프(artifact graph): 사람의 의도, 원천 문서(2), code, API, capability,
test, verification evidence를 키메타(9), 코드 앵커(18), repository evidence로 연결한 typed
graph
...
- 의미: code/API/test/config artifact의 stable ID, kind, provides, uses, implements,
...
- 사용 규칙: 모든 코드에 붙이지 않고 public API, reusable core method, domain rule,
  ...
  붙인다.
- 구분: 키메타(9)는 원천 문서(2)와 section을 위한 metadata이고, 코드 앵커는 code/test/config
...
6. 수락 이후에는 Keystone 기준서와 작업서가 최종 권위다.
...
7. Legacy uppercase `KEY-` 또는 `_KEY-` 파일은 filename migration 전까지 Keystone-managed
   문서로 읽을 수 있지만, 새 파일과 rename 대상 파일은 lower-case key prefix(11)를 사용한다.
8. key prefix(11)가 없는 일반 프로젝트 문서는 자동으로 Keystone-managed 문서로 판단하지
   않는다.
9. 외부 보조 스킬(12)은 사용자가 호출했거나 수락된 Keystone 기준서 또는 작업서가 명시적으로
   허용한 경우에만 사용한다.
...
### STD-KEYSTONE-015: 키메타와 코드 앵커는 아티팩트 그래프의 source다

키메타(9)는 원천 문서(2)와 section을 식별하고 연결하는 문서용 metadata다. 코드 앵커(18)는
code, API, test, config artifact를 아티팩트 그래프(14)에 연결하는 code-side annotation이다.
둘 다 탐색과 영향 분석을 위한 relation evidence이며, 원천 문서(2), code, test의 의미를
대체하지 않는다.

키메타(9)는 다음 필드를 중심으로 한다.

1. `key.id`: 문서 또는 section의 canonical identifier다.
2. `key.refs`: 이 문서 또는 section이 참조하는 다른 `key.id` 또는 controlled vocabulary ID
   목록이다.

코드 앵커(18)는 다음 필드를 사용할 수 있다.

1. `id`: code symbol, API contract, capability(16), test artifact, config/schema boundary의
   stable identifier다.
2. `kind`: artifact node(15)의 종류다. 초기 허용 값은 `capability`, `api`, `code_symbol`,
   `test`, `config`, `schema`다.
3. `provides`: 이 artifact가 제공하는 capability(16) 또는 contract다.
4. `uses`: 이 artifact가 사용하는 capability(16), API, code artifact다.
5. `implements`: 이 artifact가 구현하는 API contract, interface, standard requirement다.
6. `governed_by`: 이 artifact의 정책, 규칙, 의도를 정하는 기준서(3)나 결정(6)이다.
7. `verifies`: 이 artifact가 검증하는 capability(16), API, code artifact, 기준서(3) 항목이다.
8. `verified_by`: 이 artifact를 검증하는 test 또는 verification artifact다.
9. `supersedes`, `derived_from`: replacement, generation, derivation 관계를 표현한다.

권위와 한계는 다음과 같다.

1. `key.id`와 코드 앵커 `id`는 아티팩트 그래프(14) 안에서 중복되면 안 된다.
2. Typed relation은 가능한 한 이미 존재하는 `key.id`, 코드 앵커 `id`, 또는 허용된 controlled
   vocabulary ID를 참조해야 한다.
3. 같은 `key.id`를 참조하거나 typed relation으로 연결된 artifact는 함께 검토한다.
4. 같은 `key.refs`가 있어도 자동 수정하지 않는다.
5. `key.refs`, 코드 앵커, typed relation이 없어도 관련 없음으로 단정하지 않는다.
6. 실제 수정은 승인 범위, 의미 영향, source authority, artifact authority를 따져 결정한다.
7. 키메타와 코드 앵커 탐색은 제목, 경로, 링크, heading, 본문 keyword, 실제 code reference
   검색을 대체하지 않고 보완한다.
8. Declared relation도 stale할 수 있다.
9. Observed relation은 의미적 의존성을 반드시 의미하지 않는다.
10. Inferred relation은 자동 수정 근거가 아니다.

활용 규칙은 다음과 같다.

1. Keystone 작업은 문서 탐색, impact review, Context Pack 구성, worker handoff, review,
   verification을 준비할 때 키메타(9), 코드 앵커(18), repository evidence를 함께 사용한다.
2. 관련 `key.id`, `key.refs`, 코드 앵커, typed relation이 있으면 같은 ID를 참조하는 기준서(3),
   작업서(4), 진행 기록(5), 결정(6) 기록, code, API, capability(16), test artifact를 후보로
   모은다.
3. 키메타와 코드 앵커로 발견한 후보는 report나 handoff에서 확인한 evidence와 함께 드러낼 수
   있어야 한다.
4. 후보는 자동 채택, 자동 수정, 자동 acceptance 근거가 아니다. 채택 여부와 수정 여부는 적용
   기준서, 승인 범위, 실제 요구사항 충족 여부를 따져 main 또는 담당 skill이 판단한다.
5. Code-worker는 새 기능 구현 전에 capability(16), `provides`, `uses`, `implements`,
   `governed_by`, 실제 code reference, 기존 test를 함께 검색한다.
6. 새 reusable artifact를 만들 때는 기존 capability(16) provider를 재사용할 수 없는지 먼저
   확인한다.
...
7. Code, API, test artifact 후보는 코드 앵커 검색과 실제 repository 검색을 함께 사용한다.
...
12. `key.test.*`: behavior, contract, regression을 검증하는 test artifact
...
`key.id`로, code/test/config artifact는 코드 앵커(18)의 `id`로 식별한다. 파일 경로, line number,
...
2. 원천 문서(2) 변경이 capability(16), API, code, test artifact에 영향을 줄 수 있으면
   code/test impact candidate로 보고한다.
...
Change Set(17)은 하나의 사람 의도, 결정(6), 또는 work step이 문서, code, API, capability(16),
test artifact에 미치는 영향을 한 변경 단위로 묶는다.
...
11. Candidate 발견은 수정 허가가 아니다. Coordinator가 scope, role, branch context, verification을
    기준으로 후속 workflow를 조율한다.
...
2. 필요한 문서만 읽는다.
3. Assignment를 준비한다.
4. 필요한 경우 Coordinator를 통해 role-based subagent에게 작업을 routing한다.
...
### STD-KEYSTONE-032: 실행 역할은 subagent 기준서로 제한한다

Helper 또는 subagent 운영은 속도보다 책임 경계가 우선이다. 실행 역할의 lane, role,
authority, report status, 공통 금지는 `standards/subagents/key-standard-subagents.md`를
따른다.

1. Main, Author, Coordinator는 ad hoc 실행 role을 만들지 않는다.
2. 필요한 role이 기준서에 없으면 subagent 기준서를 먼저 수정하거나 main/user 결정(6)을 받는다.
3. 모든 helper/subagent는 받은 Goal 밖으로 scope를 넓히거나 추가 subagent를 생성하지 않는다.
4. Subagent output은 원천 문서(2), 기준서(3), 작업서(4), Main acceptance를 대체하지 않는다.
...
<!-- key: id=key.standard.project.std-keystone-034 refs=key.topic.skill-contract key.topic.branch-worktree key.topic.merge-gate key.topic.acceptance key.role.subagent key.topic.remote-policy key.topic.commit-checkpoint -->
...
2. 파일 수정이 필요한 subagent 작업은 session branch에서 task branch를 만들고, 병렬 실행이면
   그 task branch를 isolated worktree에 checkout해 수행한다.
...
5. Subagent, Coordinator, repo-integrator는 base branch, integration branch,
   user-facing stable branch에 직접 merge하지 않는다.
6. Merge authorization은 integration/base branch에 merge해도 된다는 사전 승인이고, final
   acceptance는 merge 후 verification까지 확인한 최종 수락이다. Merge 성공은 Git 상태가
   합쳐졌다는 뜻일 뿐 Keystone acceptance가 아니다. 진행 기록(5)의 `accepted` 상태는 Main
   또는 사용자 acceptance 이후에만 기록한다.
...
9. Branch 작업에는 `round_id`, `work_id`, `step_id`, `session_id`, `integration_owner_id`,
   `task_id`, base branch, integration branch, session branch, task branch 또는 staging branch,
   worktree path, merge target, forbidden merge target, base commit,
   `session_head_at_assignment`, `merge_target_head_at_review`, `merge_base`, remote policy,
   push 허용 여부, commit requirement, 필요한 경우 task commit range를 담은 branch context가
   필요하다.
10. Branch/worktree workflow는 기본적으로 local-only로 운영한다. Coordinator, subagent,
    repo-integrator는 remote push, remote branch 생성, PR 생성, remote merge를 수행하지
    않는다. Remote write는 사용자가 명시적으로 요청한 경우에만 가능하다.
11. 파일 수정 subagent는 task branch의 변경을 local commit으로 고정한 뒤 report한다.
    Uncommitted diff는 merge 대상이나 repo-integrator staging input으로 사용하지 않는다.
    Commit은 local checkpoint일 뿐 push 또는 Keystone acceptance가 아니다.
...
`keystone-reader`는 read-only로 원천 문서(2)와 아티팩트 그래프(14)를 읽어 project orientation,
document navigation, artifact navigation, work preparation을 지원한다. Reader는 원천 문서(2),
code, config, 진행 기록(5), 결정(6)을 수정하지 않는다.
...
결정(6) 기록을 만들거나 수정한다. 문서 수정 기준, 승인 범위, 영향 문서와 code/test impact
candidate, 키메타/index/progress 반영 범위는 Author가 소유한다. 상세 계약은
...
5. Bounded doc helper: 승인 범위가 명확하면 `doc-explorer` 또는 `doc-impact-writer` 도움을
   받을 수 있으며, role과 authority는 subagent 기준서를 따른다. File-writing helper를 직접
   사용할 때는 승인된 branch context가 먼저 준비되어야 한다.
6. Impact candidates: 문서 변경이 code, API, capability(16), test artifact에 영향을 줄 수
   있으면 후보로 보고하고, 실제 code 수정은 Coordinator workflow로 넘긴다.
7. Progress update: main acceptance 전에는 accepted 또는 complete로 표시하지 않는다.

<!-- key: id=key.standard.project.std-keystone-043 refs=key.topic.skill-contract -->
### STD-KEYSTONE-043: Coordinator는 main-supervised subagent workflow를 조율한다

`keystone-coordinator`는 작업서(4)의 Goal unit을 복구하고, main이 supervisor로 남아 있는
상태에서 formal execution workflow를 조율한다. 코드 작업은 대표적인 실행 작업이지만,
문서 작업이라도 report, review, verification, acceptance flow가 필요하면 Coordinator가
조율할 수 있다. 상세 계약은 `skills/coordinator/key-standard-coordinator.md`를 따른다.

1. Assignment: Current Step Brief, Context Pack, handoff boundary, reviewer focus를 준비한다.
2. Routing: subagent 기준서의 lane, role, authority를 기준으로 실행 역할을 선택한다.
3. 아티팩트 그래프 coordination: Change Set(17), impact candidate, reuse discovery, graph
   validation을 조율한다.
4. Report handling: report status를 progress status와 분리해 처리한다.
5. Merge coordination: merge authorization과 final acceptance를 분리한다.
6. Acceptance: main acceptance 조건을 만족한 뒤에만 진행 기록(5)을 accepted로 바꿀 수 있다.
...
### STD-KEYSTONE-051: 원천 문서(2) Git policy는 명시적이다
