...
9. Branch 작업에는 base branch, integration branch, session branch, task branch 또는
   staging branch, worktree path, merge target, forbidden merge target, base commit,
   remote policy, push 허용 여부, commit requirement, 필요한 경우 task commit range를 담은
...
10. Branch/worktree workflow는 기본적으로 local-only로 운영한다. Coordinator, subagent,
    ...
    않는다. Remote push는 사용자 또는 integration owner Main이 명시적으로 요청한 경우에만
    가능하다.
...
5. Bounded doc helper: 승인 범위가 명확하면 `doc-explorer` 또는 `doc-impact-writer` 도움을
   받을 수 있으며, role과 authority는 subagent 기준서를 따른다.
