...
<!-- key: id=key.standard.project.std-keystone-011 refs=key.topic.document-system key.topic.key-prefix -->
...
7. Keystone-managed index file은 문서 순서를 정해야 하면 `00_key-index.md`처럼 숫자와
...
<!-- key: id=key.standard.project.std-keystone-013 refs=key.topic.key-prefix key.topic.document-system -->
...
13. Slug 기준: 해당 folder 안에서 파일을 명확히 구분할 수 있는 가장 짧은 local name
14. 기존 non-key 또는 legacy uppercase `KEY` bootstrap 문서의 rename은 source path와 link를
