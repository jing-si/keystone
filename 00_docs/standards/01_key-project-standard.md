...
code/config/schema/test 사이의 연결과 영향 후보를 드러내며, worker assignment는 bounded worker가 같은
...
3. `keystone-author`: 수락된 결정(6)과 범위 안에서 원천 문서(2)와 Change Set(17)을 작성하거나
   수정한다.
4. `keystone-linker`: 문서, capability(16), code, config, schema, API, test artifact의 연결과
...
- Coordinator 사용 규칙: Keystone 작업 문맥에서 code/config/schema/test 수정 요청이 들어오면
...
- 사용 규칙: 모든 코드에 붙이지 않고 public API, reusable core method, domain rule,
  ...
  붙인다.
- 구분: 키메타(9)는 원천 문서(2)와 section을 위한 metadata이고, 코드 앵커는 code/config/schema/test
...
6. 문서, capability(16), code, config, schema, API, test artifact의 연결, impact candidate,
   stale metadata 후보 탐색은 `keystone-linker`로 routing한다.
...
8. Keystone 작업 문맥에서 code/config/schema/test 수정 요청이 들어오면 명시적 `$coordinator` 호출이
...
9. 문서, capability(16), code, config, schema, API, test artifact의 impact/stale 후보 해석은
...
`key.id`로, code/config/schema/test artifact는 코드 앵커(18)의 `id`로 식별한다. 파일 경로, line number,
...
2. 원천 문서(2) 변경이 capability(16), API, code, config, schema, test artifact에 영향을 줄 수
   있으면 code/config/schema/test impact candidate로 보고한다.
...
키메타/index/progress 반영 범위는 Author가 소유한다. code/config/schema/test impact candidate의
...
6. Change Set: 큰 문서 변경, code/config/schema/test 영향이 있는 문서 변경, 여러 artifact에 걸친 변경은
...
함께 사용해 문서, capability(16), code, config, schema, API, test artifact의 연결과 영향 후보를
탐색한다.
...
5. Handoff: 문서 metadata 수정은 Author에게, code/config/schema/test metadata 수정이나
...
5. Artifact linking
   - Linker는 문서, capability(16), code, config, schema, API, test artifact의 영향 후보와 stale 후보를
     read-only로 보고한다.
6. Work execution
