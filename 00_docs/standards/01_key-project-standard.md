...
<!-- key: id=key.standard.project.std-keystone-015 refs=key.topic.document-system key.doc.source key.topic.keystone-metadata key.topic.work-sequence -->
### STD-KEYSTONE-015: 키스톤 메타데이터(9)는 identity와 relation을 표현한다
...
1. Keystone 작업은 문서 탐색, impact review, Context Pack 구성, worker handoff, review,
   verification을 준비할 때 키스톤 메타데이터(9)를 우선 탐색 단서로 사용한다.
2. 관련 `key.id`와 `key.refs`가 있으면 같은 ID를 참조하는 기준서(3), 작업서(4), 진행
   기록(5), 결정(6) 기록을 후보로 모은다.
3. Metadata로 발견한 후보는 report나 handoff에서 확인한 evidence와 함께 드러낼 수 있어야
   한다.
4. Metadata 후보는 자동 채택, 자동 수정, 자동 acceptance 근거가 아니다. 채택 여부와 수정
   여부는 적용 기준서, 승인 범위, 실제 요구사항 충족 여부를 따져 main 또는 담당 skill이
   판단한다.
...
navigation, work preparation을 지원한다. Reader는 원천 문서(2), code, config, 진행 기록(5),
결정(6)을 수정하지 않는다.
...
4. Merge coordination: merge authorization과 final acceptance를 분리한다.
5. Acceptance: main acceptance 조건을 만족한 뒤에만 진행 기록(5)을 accepted로 바꿀 수 있다.
