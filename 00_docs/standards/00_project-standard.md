...
3. Superpowers 같은 외부 workflow 스킬은 사용자가 호출했거나 수락된 Keystone 기준서 또는
...
4. 작업서 tree도 node별 `00_index.md`를 가진다.
...
4. Decisions file: 필요할 때 `decisions.md`
5. Slug 기준: 해당 folder 안에서 파일을 명확히 구분할 수 있는 가장 짧은 local name
...
상세 계약은 `skills/clarify/standard-clarify.md`를 따른다.
...
결정(6) 기록을 만들거나 수정한다. 상세 계약은 `skills/author/standard-author.md`를 따른다.
...
상세 계약은 `skills/coordinator/standard-coordinator.md`를 따른다.
...
1. Project orientation
   - Main agent는 `context-map.md`를 확인해 active standards와 active work를 찾는다.
   - 현재 active work는 `00_docs/works/00_index.md`에서 찾는다.
2. Document reading
