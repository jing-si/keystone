...
1. 기본적으로 생성하지 않는다.
2. 이미 존재하고 명확히 영향받는 파생 에이전트 문서(8)만 함께 업데이트한다.
3. 새로 생성하는 경우는 사용자 명시 요청, 복잡한 handoff 사전 검토, 같은 step rerun,
   audit record 필요처럼 명시적 필요가 있을 때로 제한한다.
4. 충돌이 있으면 원천 문서가 항상 우선한다.
...
3. 문서 작업 진행 기록: 기준서(3), 작업서(4), 결정(6) 기록, child 기준서 초안처럼 작업
   복구에 의미 있는 문서 단위 변화만 진행 기록(5)에 남긴다.
4. 기록하지 않는 변경: 단순 오탈자, 미세 표현 수정, 의미 변화 없는 formatting은 진행
   기록(5)에 남기지 않는다.
...
`keystone-clarify`는 topic-scoped high-impact 결정(6)을 수집하고, 수락된 decision
summary와 edit plan을 바탕으로 `keystone-author`가 관련 원천 문서(2)를 업데이트할 수
있게 준비한다. 상세 계약은 `skills/clarify/standard-clarify.md`를 따른다.

1. Plan Mode
   - 한 topic에 필요한 질문과 결정(6)을 수집한다.
2. Default Mode
   - 수락된 decision summary와 edit plan을 Author가 적용할 수 있게 정리한다.
   - 다른 문서에 영향이 없는 현재 문서의 단순 오탈자만 직접 수정할 수 있다.
3. Child 기준서
   - Trigger, non-trigger, setup question, reflection, impact update, stop condition은
     `skills/clarify/standard-clarify.md`를 따른다.
...
결정(6) 기록을 만들거나 수정한다. 상세 계약은 `skills/author/standard-author.md`를
따른다.

1. Creation
   - 새 기준서(3), 작업서(4), 작업 패키지 문서, index를 만든다.
2. Revision
   - 승인된 범위 안에서 기존 원천 문서(2)를 수정한다.
3. Clarify result 적용
   - 수락된 decision summary와 edit plan을 관련 원천 문서(2)에 반영하는 기본 담당자다.
4. Child 기준서
   - Trigger, non-trigger, mode, progress update boundary, stop condition은
     `skills/author/standard-author.md`를 따른다.
...
상태에서 role 기반 subagent 작업, report, review, verification, acceptance flow를
조율한다. 상세 계약은 `skills/coordinator/standard-coordinator.md`를 따른다.

1. Assignment
   - Current Step Brief, Context Pack, worker handoff boundary, reviewer focus를 준비한다.
2. Routing
   - Explorer, worker/code modifier, reviewer, verifier 같은 role을 bounded Goal에 맞춰
     선택한다.
3. Report handling
   - Subagent report status를 진행 상태와 분리해 accept, repair, verify, escalate 여부를
     판단한다.
4. Acceptance
   - Main acceptance 조건을 만족한 뒤에만 진행 기록(5)을 accepted 또는 완료 상태로 바꿀 수
     있다.
5. Child 기준서
   - Trigger, non-trigger, runtime output, role routing, report handling, stop condition은
     `skills/coordinator/standard-coordinator.md`를 따른다.
...
6. Supporting workflow
   - Superpowers는 Keystone 원천 문서(2)를 대체하지 않는 명시적 supporting
     workflow로 사용할 수 있다.
7. Document policy
   - 파생 에이전트 문서(8)는 `STD-KEYSTONE-020`의 조건을 만족할 때만 생성하거나
     업데이트할 수 있으며 원천 문서가 되지는 않는다.
