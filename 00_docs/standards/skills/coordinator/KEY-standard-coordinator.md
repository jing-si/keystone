...
<!-- tags: coordinator, subagent, 작업실행 -->
## 목적
...
<!-- tags: coordinator, 작업실행, subagent -->
## Trigger condition
...
<!-- tags: coordinator, 작업실행, verification, acceptance -->
## Common workflow
...
5. Current work나 관련 기준서에 태그(9)가 있으면 같은 태그의 문서를 Context Pack 후보로
   확인한다.
6. Work unit이 subagent-sized이고 scope가 bounded인지 판단한다.
7. Role을 선택하거나 main override를 반영한다.
...
<!-- tags: coordinator, output-contract, 작업실행 -->
...
2. Context Pack
   - worker나 reviewer가 읽어야 하는 관련 기준서(3), 작업서(4), 결정(6), scope,
     progress 발견사항, 태그 기반 관련 문서 후보를 담는다.
3. Worker handoff
...
4. Reviewer brief
   - worker goal, completion criteria, changed files 또는 문서 output, known risks,
     verification result, 같은 태그 문서와의 충돌 여부, reviewer focus를 담는다.
...
1. Explorer
   - read-only 조사, 기존 규칙 중복 확인, repository 조사, prototype 비교가 필요할 때
     사용한다. 태그 기반 관련 문서 탐색이 필요할 때도 사용할 수 있다.
...
<!-- tags: coordinator, report, subagent -->
...
<!-- tags: coordinator, verification, review -->
...
3. 같은 태그 문서와의 충돌 여부를 reviewer focus 후보로 확인한다.
...
<!-- tags: coordinator, acceptance, verification -->
## Acceptance contract
...
<!-- tags: coordinator, progress-update, acceptance -->
...
<!-- tags: coordinator, verification, acceptance -->
## Verification
...
8. 태그 기반 후보 문서를 Context Pack 후보와 reviewer focus 후보로 사용할 수 있어야 한다.
