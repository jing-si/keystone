...
7. 사용자의 답이 decision을 확정하기에 부족하면 `author_edit_ready: false`, unresolved
   `open_questions`, Author-ready `edit_plan` 없음으로 보고한다.
8. `acceptance_or_status_impact`가 있으면 progress status, acceptance criteria, status semantics를
   바꾸는지 분리해 보고한다.
...
3. 금지:
   ...
   - 이 Keystone implementation repository에서는 fixture/test step 또는 명시적인 project-local
     setup 요구가 없는 한 `.keystone/config.yaml`을 생성하지 않기
...
7. Implementation, config, schema, API, test, auth/security, generated/codegen, publishing decision이
   먼저 필요하다.
