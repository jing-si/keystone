...
2. `context-map.md`가 있으면 먼저 읽어 active standards와 work package를 파악한다.
3. `standards/00_index.md` 또는 해당 project의 standards index를 읽는다.
4. 현재 요청과 관련된 child index만 따라 내려가며 관련 없는 branch를 읽지 않는다.
5. active work가 있으면 해당 작업서(4)와 진행 기록(5)을 읽는다.
6. 필요한 경우 shallow repository snapshot을 만든다.
7. output에는 항상 `sources_read`, `assumptions`, `risks_and_gaps`,
