...
<!-- key: id=key.standard.skill.reader.standard-relations refs=key.role.reader key.doc.standard key.topic.skill-contract key.topic.work-round -->
## 기준 관계
...
2. 상위 규칙: `STD-KEYSTONE-001`, `STD-KEYSTONE-010`, `STD-KEYSTONE-011`,
   ...
   `STD-KEYSTONE-026`, `STD-KEYSTONE-040`
...
<!-- key: id=key.standard.skill.reader.common-workflow refs=key.role.reader key.topic.document-navigation key.topic.work-preparation key.topic.work-round -->
## Common workflow
...
8. 요청 또는 current work에 관련 키스톤 메타데이터(9)가 있으면 같은 `key.id`를 참조하는
   문서를 탐색 후보로 확인한다.
9. Metadata 탐색은 link, path, heading, keyword 탐색과 함께 사용한다.
10. 필요한 경우 shallow repository snapshot을 만든다.
11. output에는 항상 `sources_read`, `assumptions`, `risks_and_gaps`,
...
<!-- key: id=key.standard.skill.reader.navigator-mode refs=key.role.reader key.topic.document-navigation key.contract.output key.topic.work-round -->
### Navigator Mode
...
<!-- key: id=key.standard.skill.reader.work-prep-mode refs=key.role.reader key.topic.work-preparation key.contract.output key.topic.work-round -->
### Work Prep Mode
...
2. 읽기 범위: 관련 기준서(3), root works index, active round index, work node index,
   작업서(4), 진행 기록(5), completion criteria, verification note
3. Output field:
   - `active_round`
   - `round_id`
   - `round_index`
