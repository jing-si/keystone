...
<!-- tags: reader, 문서탐색, 작업준비, output-contract -->
## 적용 범위
...
5. 요청 또는 current work에 관련 태그(9)가 있으면 같은 태그를 가진 문서를 탐색 후보로
   확인한다.
6. 태그 탐색은 link, path, heading, keyword 탐색과 함께 사용한다.
...
<!-- tags: reader, output-contract, 작업준비 -->
## Mode contract
...
<!-- tags: reader, 문서탐색, output-contract -->
### Navigator Mode
...
<!-- tags: reader, 작업준비, output-contract -->
### Work Prep Mode
...
Reader는 태그(9)를 관련 문서 탐색 단서로 사용한다.

1. 같은 태그가 있는 문서는 관련 문서 후보로 검토한다.
2. 같은 태그가 있어도 자동으로 관련 문서라고 확정하지 않는다.
3. 태그가 없어도 관련 없음으로 단정하지 않는다.
4. Navigator Mode와 Work Prep Mode output에는 사용한 태그와 태그로 발견한 후보 문서를
   포함한다.
5. 태그 탐색 결과는 title, path, link, heading, keyword 근거와 함께 판단한다.

<!-- tags: reader, read-only, 작업준비 -->
## Stop condition
...
<!-- tags: reader, output-contract, 검증 -->
## Verification
...
4. Work Prep Mode output은 coordinator가 final handoff를 만들기 전 단계로 사용할 수
   있어야 한다.
5. 태그를 사용한 경우 output에서 사용한 태그와 후보 문서를 확인할 수 있어야 한다.
