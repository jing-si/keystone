...
이 기준서는 `keystone-reader`의 상세 계약을 정의한다. `keystone-reader`는 원천 문서(2)를
읽어 프로젝트를 파악하고, 관련 기준서(3)와 작업서(4)를 찾고, main이 다음 작업을 준비할
수 있도록 read-only context를 만든다.

<!-- key: id=key.standard.skill.reader.scope refs=key.role.reader key.topic.document-navigation key.topic.work-preparation key.contract.output -->
## 적용 범위

1. `keystone-reader`의 trigger와 non-trigger condition
2. Orientation, Navigator, Work Prep mode의 workflow와 output
3. 문서 root, standards/works tree, active work 탐색 방식
4. Repository snapshot과 document-to-repository mismatch reporting
5. Reader의 read-only boundary, stop condition, verification 기준
...
2. Primary role: read-only project orientation, document navigation, work preparation
...
5. document-to-repository mismatch를 read-only로 확인하고 보고해야 한다.
...
<!-- key: id=key.standard.skill.reader.common-workflow refs=key.role.reader key.topic.document-navigation key.topic.work-preparation -->
## Common workflow

Reader는 mode와 무관하게 다음 순서를 따른다.

1. 설정된 document root(1)를 `STD-KEYSTONE-015`에 따라 해석한다.
...
5. 요청 또는 current work에 관련 키스톤 메타데이터(9)가 있으면 같은 `key.id`를 참조하는
   문서를 탐색 후보로 확인한다.
6. Metadata 탐색은 link, path, heading, keyword 탐색과 함께 사용한다.
7. active work가 있으면 해당 작업서(4)와 진행 기록(5)을 읽는다.
8. 필요한 경우 shallow repository snapshot을 만든다.
9. output에는 항상 `sources_read`, `assumptions`, `risks_and_gaps`,
...
<!-- key: id=key.standard.skill.reader.orientation-mode refs=key.role.reader key.topic.document-navigation -->
### Orientation Mode

Orientation Mode는 project-level overview를 만든다.

1. 읽기 범위: instruction, 설정된 document root(1), context map, standards/works index,
   active work, shallow repository snapshot
...
<!-- key: id=key.standard.skill.reader.navigator-mode refs=key.role.reader key.topic.document-navigation key.contract.output -->
### Navigator Mode

Navigator Mode는 현재 요청과 관련된 원천 문서(2)를 찾는다.

1. Input: 사용자 요청, current task, known document root
2. 읽기 범위: standards/works index, parent 기준서, 관련 child 기준서, active work
3. Output field:
   - `request_summary`
   - `key_ref_queries`
   - `matched_documents`
...
<!-- key: id=key.standard.skill.reader.work-prep-mode refs=key.role.reader key.topic.work-preparation key.contract.output -->
### Work Prep Mode
...
2. 읽기 범위: 관련 기준서(3), 작업서(4), 진행 기록(5), completion criteria,
   verification note
3. Output field:
   - `goal`
   - `source_context`
...
<!-- key: id=key.standard.skill.reader.mismatch-handling refs=key.role.reader key.topic.document-navigation key.boundary.read-only -->
## Mismatch handling

원천 문서(2)와 repository state가 충돌하면 Reader는 다음처럼 처리한다.

1. 자동 수정하지 않는다.
2. mismatch의 source와 evidence를 기록한다.
3. 영향을 받는 문서와 작업을 식별한다.
...
<!-- key: id=key.standard.skill.reader.metadata-handling refs=key.role.reader key.topic.document-navigation key.topic.keystone-metadata -->
## Keystone metadata handling

Reader는 키스톤 메타데이터(9)를 관련 문서 탐색 단서로 사용한다.

1. 같은 `key.id`를 참조하는 문서는 관련 문서 후보로 검토한다.
2. 같은 `key.refs`가 있어도 자동으로 관련 문서라고 확정하지 않는다.
3. `key.refs`가 없어도 관련 없음으로 단정하지 않는다.
4. Navigator Mode와 Work Prep Mode output에는 사용한 `key.refs`와 metadata로 발견한 후보
   문서를 포함한다.
5. Metadata 탐색 결과는 title, path, link, heading, keyword 근거와 함께 판단한다.
...
2. current step, completion criteria, applicable standard를 복구할 수 없다.
...
6. document-to-repository mismatch가 current task의 방향을 바꿀 수 있다.
...
2. Mode별 output field가 `STD-KEYSTONE-019`의 상위 mode 책임과 일관되어야 한다.
3. Reader output은 원천 문서(2)를 대체하거나 수정하지 않아야 한다.
4. Work Prep Mode output은 coordinator가 final handoff를 만들기 전 단계로 사용할 수
   있어야 한다.
5. 키스톤 메타데이터를 사용한 경우 output에서 사용한 `key.refs`와 후보 문서를 확인할 수
   있어야 한다.
