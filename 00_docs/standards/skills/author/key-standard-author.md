...
<!-- key: id=key.standard.skill.author.common-workflow refs=key.role.author key.topic.document-authoring key.topic.impact-review key.topic.author-edit-contract key.topic.doc-helper key.standard.subagent key.topic.branch-worktree key.topic.artifact-graph -->
## Common workflow
...
13. Helper 작업에 branch/worktree provisioning, merge, reviewer/verifier, 여러 writer, merge gate가
    필요하다.
14. 문서 변경이 code/API/test artifact 수정을 요구한다.
