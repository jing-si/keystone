...
7. 큰 문서 수정 전에 대상, 범위, 영향 문서, 검증 기준을 Author Edit Contract로 정리해야
   한다.
8. 승인된 문서 수정 범위 안에서 subagent 기준서의 `docs` lane helper인 `doc-explorer` 또는
...
<!-- key: id=key.standard.skill.author.required-input refs=key.role.author key.topic.document-authoring key.topic.branch-worktree -->
## Required input
...
8. File-writing helper를 직접 사용할 경우 승인된 branch context

Input이 부족하고 잘못된 문서 변경으로 이어질 위험이 있으면 Author는 수정하지 않고
main/user에게 결정(6)을 요청하거나 `keystone-clarify`가 다룰 topic으로 보고해야 한다.

<!-- key: id=key.standard.skill.author.common-workflow refs=key.role.author key.topic.document-authoring key.topic.impact-review key.topic.author-edit-contract key.topic.doc-helper key.standard.subagent key.topic.branch-worktree -->
## Common workflow
...
10. 영향 후보가 여러 원천 문서(2)에 걸치거나 수정 누락 시 문서 간 불일치가 생길 수 있으면
    `doc-explorer`로 후보를 조사하거나 `doc-impact-writer` 사용 여부를 판단한다.
11. `doc-impact-writer`는 Author Edit Contract 안에서 필요한 관련 문서만 수정한다. 연결된
    문서라는 이유만으로 모두 수정하지 않는다.
12. File-writing helper를 직접 사용할 때는 Main 또는 Author가 승인된 branch context를 먼저
    준비해야 한다.
13. Author가 직접 helper를 쓰는 경우는 하나의 bounded writer, 준비된 task branch/worktree,
    formal reviewer/verifier 없음, merge gate 불필요 조건을 모두 만족해야 한다.
14. Reviewer/verifier, 여러 writer, branch/worktree provisioning, merge, merge gate가 필요하면
...
18. 키스톤 메타데이터(9)가 있는 경우 같은 `key.id`를 참조하는 문서를 impact candidate로
    검토한다.
19. 문서 link, 용어, 구조, Markdown 형식을 검증한다.
20. 변경 파일, 적용한 결정(6), 검증 결과, 남은 risk를 보고한다.
...
12. File-writing helper를 직접 사용해야 하지만 승인된 branch context가 없다.
13. Helper 작업에 branch/worktree provisioning, merge, reviewer/verifier, 여러 writer, merge gate가
    필요하다.
...
7. 키스톤 메타데이터가 있는 문서를 수정할 때 같은 `key.id`를 참조하는 impact candidate를
   검토했는지 확인할 수 있어야 한다.
8. `doc-explorer` 또는 `doc-impact-writer` helper를 사용해도 subagent 기준서, 승인 범위,
