...
(9) 키스톤 메타데이터(Keystone metadata): 원천 문서(2)와 section을 식별하고 연결하기 위한
구조화 메타데이터

- 의미: `key.id`로 자기 자신의 canonical identifier를 선언하고, `key.refs`로 관련
  `key.id`를 참조해 문서 graph를 만든다.
- 권위 규칙: `STD-KEYSTONE-031`
- 사용 규칙: 같은 `key.id`를 참조하는 문서는 함께 검토할 후보가 되지만 자동 수정 대상은
  아니다.
...
<!-- key: id=key.standard.project.std-keystone-002 refs=key.doc.source key.topic.document-system -->
...
<!-- key: id=key.standard.project.std-keystone-003 refs=key.topic.document-system key.topic.skill-contract -->
...
<!-- key: id=key.standard.project.std-keystone-004 refs=key.topic.document-system key.topic.skill-contract -->
...
<!-- key: id=key.standard.project.std-keystone-005 refs=key.topic.skill-contract key.topic.formal-workflow key.topic.acceptance -->
### STD-KEYSTONE-005: Main은 최종 조율과 수락 책임을 가진다
...
<!-- key: id=key.standard.project.std-keystone-006 refs=key.doc.source key.topic.document-system -->
...
<!-- key: id=key.standard.project.std-keystone-009 refs=key.topic.key-prefix key.topic.skill-contract -->
...
7. KEY prefix(11)가 없는 일반 프로젝트 문서는 자동으로 Keystone-managed 문서로 판단하지
   않는다.
8. 외부 보조 스킬(12)은 사용자가 호출했거나 수락된 Keystone 기준서 또는 작업서가 명시적으로
   허용한 경우에만 사용한다.

<!-- key: id=key.standard.project.std-keystone-010 refs=key.doc.source key.topic.document-system -->
...
<!-- key: id=key.standard.project.std-keystone-011 refs=key.topic.document-system key.topic.skill-contract -->
...
<!-- key: id=key.standard.project.std-keystone-012 refs=key.topic.skill-contract -->
...
<!-- key: id=key.standard.project.std-keystone-013 refs=key.topic.skill-contract key.topic.formal-workflow key.role.subagent -->
### STD-KEYSTONE-013: 실행 역할은 lane, role, authority로 제한한다

Helper 또는 subagent 운영은 속도보다 책임 경계가 우선이다. 실행 역할은 다음 세 축으로
제한한다.

1. Lane: `docs`, `code`, `repo`처럼 작업 영역을 구분한다.
2. Role: `explorer`, `doc-writer`, `code-worker`, `reviewer`, `verifier`처럼 수행할 일을
   구분한다.
...
2. `doc-writer`: Author가 정한 승인 범위 안의 bounded 문서 수정
3. `code-worker`: Coordinator가 정한 승인 범위 안의 bounded 코드 작업
4. `reviewer`: completion criteria, scope, risk, regression 가능성 검토
5. `verifier`: 기준서-led verification 실행 또는 failure 원인 분리

모든 helper/subagent는 받은 Goal 밖으로 scope를 넓히거나 추가 helper/subagent를 생성하지
않는다.

<!-- key: id=key.standard.project.std-keystone-014 refs=key.topic.skill-contract key.topic.external-assist key.topic.document-system -->
### STD-KEYSTONE-014: 외부 보조 스킬은 선택적 content-assist layer다
...
<!-- key: id=key.standard.project.std-keystone-015 refs=key.topic.document-system key.doc.source -->
...
<!-- key: id=key.standard.project.std-keystone-016 refs=key.topic.document-system key.topic.key-prefix -->
...
<!-- key: id=key.standard.project.std-keystone-017 refs=key.topic.key-prefix key.topic.document-system -->
...
<!-- key: id=key.standard.project.std-keystone-018 refs=key.topic.document-system key.topic.skill-contract -->
...
<!-- key: id=key.standard.project.std-keystone-019 refs=key.topic.skill-contract key.doc.source -->
...
<!-- key: id=key.standard.project.std-keystone-021 refs=key.doc.source key.topic.document-system -->
...
<!-- key: id=key.standard.project.std-keystone-023 refs=key.topic.skill-contract key.topic.document-system -->
...
<!-- key: id=key.standard.project.std-keystone-024 refs=key.topic.skill-contract -->
...
<!-- key: id=key.standard.project.std-keystone-025 refs=key.topic.skill-contract key.doc.source -->
...
<!-- key: id=key.standard.project.std-keystone-026 refs=key.topic.document-system key.doc.source -->
...
<!-- key: id=key.standard.project.std-keystone-027 refs=key.doc.source key.topic.document-system -->
...
<!-- key: id=key.standard.project.std-keystone-028 refs=key.topic.document-system -->
...
<!-- key: id=key.standard.project.std-keystone-029 refs=key.topic.skill-contract key.doc.source -->
### STD-KEYSTONE-029: Author는 승인된 원천 문서(2) 작성을 담당한다

`keystone-author`는 수락된 요청과 결정(6)을 바탕으로 기준서(3), 작업서(4), 진행 기록(5),
결정(6) 기록을 만들거나 수정한다. 문서 수정 기준, 승인 범위, 영향 문서,
metadata/index/progress 반영 범위는 Author가 소유한다. 상세 계약은
...
5. Bounded doc helper: 승인 범위가 명확하면 `doc-explorer` 또는 `doc-writer` 도움을 받을 수
   있다.
6. Progress update: main acceptance 전에는 accepted 또는 complete로 표시하지 않는다.

<!-- key: id=key.standard.project.std-keystone-030 refs=key.topic.skill-contract -->
...
2. Routing: lane, role, authority를 기준으로 explorer, doc-writer, code-worker, reviewer,
   verifier 같은 실행 역할을 선택한다.
...
<!-- key: id=key.standard.project.std-keystone-031 refs=key.topic.document-system key.doc.source key.topic.keystone-metadata key.topic.work-sequence -->
### STD-KEYSTONE-031: 키스톤 메타데이터(9)는 identity와 relation을 표현한다

키스톤 메타데이터(9)는 기준서(3), 작업서(4), 진행 기록(5), 결정(6) 기록을 수정하거나 읽을 때
문서와 section을 식별하고 관련 문서를 찾기 위한 구조화 metadata다. 키스톤 메타데이터는
원천 문서(2)의 의미를 대체하지 않는다.

키스톤 메타데이터는 두 필드를 가진다.

1. `key.id`: 문서, section, 또는 기준서가 허용한 controlled vocabulary의 canonical
   identifier다.
...
1. `key.id`는 document root(1) 안에서 중복되면 안 된다.
2. `key.refs`는 가능한 한 이미 존재하는 `key.id` 또는 허용된 controlled vocabulary ID를
   참조해야 한다.
3. 같은 `key.id`를 참조하는 문서는 함께 검토한다.
4. 같은 `key.refs`가 있어도 자동 수정하지 않는다.
5. `key.refs`가 없어도 관련 없음으로 단정하지 않는다.
6. 실제 수정은 승인 범위, 의미 영향, source authority를 따져 결정한다.
7. Metadata 탐색은 제목, 경로, 링크, heading, 본문 keyword 검색을 대체하지 않고 보완한다.

Metadata 기반 문서 해석 순서는 다음 원칙을 따른다.

1. `KEY-context-map.md`가 있으면 먼저 읽어 active standards와 active work를 찾는다.
2. 기준서(3)는 최상위 기준서에서 시작해 필요한 child 기준서로 내려간다.
3. 작업서(4)는 최상위 work index에서 시작해 active work node 또는 명시된 work node로 내려간다.
4. Current work node 안에서는 node index, 작업서(4), 진행 기록(5)을 함께 읽어 현재 step과
   실행 범위를 복구한다.
5. 결정(6) 기록, 범위 문서(7), 파생 에이전트 문서(8), metadata로 발견한 후보 문서는 위
   순서를 보강하는 추가 서류로 읽는다.
6. 사용자가 특정 step이나 work를 직접 지정하면 해당 work node를 찾을 수 있지만, 실행 판단은
   적용 가능한 기준서(3)를 함께 읽은 뒤에 한다.
7. `key.topic.current-step`으로 표시된 문맥은 `key.topic.next-step` 같은 후속 참고 문맥보다
   우선한다.
8. 문서 frontmatter의 `key.refs`는 section-level `key.refs`보다 넓은 관련성을 나타낸다.
...
1. 문서 전체에 적용되는 metadata는 YAML frontmatter의 `key`에 둔다.
...
2. 특정 section에만 적용되는 metadata는 heading 바로 위 Markdown comment로 한 줄에 둔다.
...
3. Section-level metadata는 바로 뒤 heading의 section에 적용된다.
4. Section `key.id`는 가능하면 문서 `key.id`를 prefix로 사용한다.
5. `key.refs`는 느슨한 keyword가 아니라 문서 graph의 edge로 취급한다.

<!-- key: id=key.standard.project.std-keystone-032 refs=key.topic.bootstrap key.doc.source key.topic.skill-contract -->
...
<!-- key: id=key.standard.project.active-work refs=key.topic.document-system -->
## 관련 active work
