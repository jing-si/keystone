...
`00_docs/standards/01_key-project-standard.md`를 현재 `works/` 구조와 다섯 Keystone 스킬 역할에
맞게 parent 기준서로 정리한다.
...
- 다섯 스킬의 상위 역할
- 외부 보조 스킬과 외부 코딩 스킬의 Coordinator injected skill 경계
...
- Skill source 생성
- 후속 step progress 변경
- 외부 보조 스킬을 Keystone runtime dependency로 고정하기
...
- [ ] 외부 코딩 스킬은 Coordinator를 대체하지 않고 injected skill로 쓰인다는 상위 정책이 명확하다.
...
- `rg -n "external coding skill|외부 코딩 스킬|injected skill|DEC-WORKS-008" 00_docs/standards 00_docs/works`
- Project Standard와 standards/work index의 link consistency 확인
...
- 외부 보조 스킬 정책이 Coordinator, Subagent 기준서와 충돌하지 않는지 확인한다.
