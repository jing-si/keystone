...
- 기준서(3) 작성 규칙
- 작업서(4) 생성 표준
- Author Edit Contract shape과 승인 범위 표시
...
- [ ] Author Edit Contract가 target document, affected candidate, forbidden change,
      progress update policy, verification을 구분한다.
- [ ] Author patch report가 edit, no-edit candidate, progress update, verification, residual
      risk를 복구할 수 있게 한다.
...
- `STD-KEYSTONE-042`
- `DEC-WORKS-002`
- Author Edit Contract shape
- Author patch report shape
...
- Author Edit Contract 없이 여러 원천 문서(2)를 의미 있게 수정해야 한다.
- Author가 code/config/schema/API/test artifact 또는 generated output을 수정해야 한다.
...
- `rg -n "author_edit_contract|target_documents|affected_document_candidates|progress_update_policy" 00_docs/standards/skills/author/key-standard-author.md`
- `rg -n "author_patch_report|no_edit_candidates|progress_updates|residual_risk" 00_docs/standards/skills/author/key-standard-author.md`
...
- Author Edit Contract가 후보 검토와 실제 수정 권한을 분리하는지 확인한다.
