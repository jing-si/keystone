...
`keystone-linker`의 read-only artifact discovery, impact analysis, stale review, execution
context seed 계약을 정리한다.
...
- Artifact Discovery, Impact Analysis, Stale Review, Execution Context Seed mode
...
- [ ] `linker_report`가 Coordinator execution packet의 artifact context seed로 사용될 수 있다.
...
Linker는 artifact graph 해석과 후보 보고에 집중한다. 문서 반영은 Author, execution packet과
실행 회수는 Coordinator 책임으로 남긴다.
...
- Coordinator execution packet seed
...
- Execution packet 배정 책임이 Linker로 넘어간다.
