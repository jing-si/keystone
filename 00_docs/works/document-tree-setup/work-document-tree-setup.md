...
`00_docs/works/00_index.md`에서 관리하도록 정리한다.
...
- `00_docs/context-map.md`와 README의 active work link 보정
...
- [ ] 각 work node는 `00_index.md`, `work-{slug}.md`, `progress.md`를 가진다.
- [ ] 실행 순서는 `00_docs/works/00_index.md`에서 확인된다.
...
- `works/00_index.md`가 순서를 관리한다는 결정
...
진행 상태는 `progress.md`에 기록한다. S00은 main acceptance 후에만 accepted로 표시한다.
